...
class NewGroup(QDialog):
    createSuccessSig = pyqtSignal(int, int)
    ...
    def setupUi(self):
        self.setObjectName("createGroup")
        self.resize(416, 137)
    ...
    def retranslateUi(self):
        ...
        self.setWindowTitle(_translate("joinGroup", "请输入新创的群ID"))
    ...
    def on_confirm_clicked(self):
        ...
        if len(room_id) is 0:
            ...
            res = db.createGroup(int(room_id), self.token)
            if res is True:
                QMessageBox.about(None, '标题', "创建成功")
                # 吐出信号主窗口刷新
                self.createSuccessSig.emit(int(room_id), 1)
                self.close()
            else:
